...
const InputSchema = z.object({
  ...
  plan: z.enum(["phone_starter", "phone_email", "ai_front_office"]),
...
export const sendSelfServeInvite = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const greetingName = data.firstName ? ` ${data.firstName}` : data.businessName ? ` ${data.businessName}` : "";
...
<html><body style="font-family:Arial,sans-serif;background:#ffffff;color:#111;padding:24px;">
  <div style="max-width:560px;margin:0 auto;">
    ...
    <p>Thanks for choosing the <strong>${planLabel}</strong> plan with Vektiss.</p>
