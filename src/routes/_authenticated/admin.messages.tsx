...
function AdminMessagesPage() {
  ...
  // Fetch all SMS messages (super admin — no tenant filter)
  const { data: messages = [], isLoading, refetch } = useQuery({
    queryKey: ["admin-sms-messages"],
    queryFn: () => listSmsMessages(null),
  });
  ...
  const deleteMut = useMutation({
    mutationFn: deleteSmsMessage,
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-sms-messages"] });
      toast.success("Message removed from log");
    },
    onError: () => toast.error("Failed to delete message"),
  });

  const statusMut = useMutation({
    mutationFn: ({ id, status }: { id: string; status: SmsStatus }) =>
  ...
  const stats = useMemo(() => {
    ...
    const deliveryRate =
      total > 0 ? Math.round(((delivered + (total - delivered - failed)) / total) * 100) : 0;
    return { total, delivered, failed, intakeForms, last7, deliveryRate };
  }, [messages]);
  ...
  const filtered = useMemo(() => {
    return messages.filter((m) => {
      ...
      const matchTenant =
        tenantFilter === "all" || m.tenant_id === tenantFilter;
      return matchSearch && matchType && matchStatus && matchTenant;
    });
  }, [messages, search, typeFilter, statusFilter, tenantFilter]);
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 px-6 py-16 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {messages.length === 0
                  ? "No SMS messages have been sent yet. They'll appear here automatically once your AI receptionist starts texting callers."
                ...
                {filtered.map((msg) => {
                  ...
                  return (
                    <>
                      ...
                      {isExpanded && (
                        <TableRow key={`${msg.id}-expanded`} className="bg-muted/20">
        ...
        <DialogContent className="max-w-lg">
          ...
          {selectedMsg && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <p className="font-medium">
                    {selectedMsg.caller_name ?? "Unknown"}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {selectedMsg.to_phone}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Client</p>
                  <p className="font-medium">
                    {tenantName(selectedMsg.tenant_id)}
                  </p>
                </div>
                <div>
                ...
                {selectedMsg.delivered_at && (
                  <div>
                    ...
                    <p>
                      {new Date(selectedMsg.delivered_at).toLocaleString()}
                    </p>
                  </div>
                )}
              </div>
              ...
              {selectedMsg.twilio_sid && (
                <div>
                  <p className="mb-1 text-xs text-muted-foreground">
                    Twilio SID
                  </p>
                  <p className="font-mono text-xs">{selectedMsg.twilio_sid}</p>
                </div>
              )}
